            ...
            if (statsContainer) {
                if (data.topCards && data.topCards.length > 0) {
                    statsContainer.innerHTML = data.topCards.map(stat => {
                        ...
                        return `<div class="stat-tile"><div class="stat-emoji">${cardDisplay}</div><div class="stat-cat">${window.escHtml(stat.category)}</div><div class="stat-count">${window.escHtml(String(stat.max_matches))}</div></div>`;                    }).join('');
                } else {
